const STORAGE_KEY="alku_depo_v7";
const LEGACY_KEYS=["alku_depo_v6","alku_depo_v5","alku_depo_v4","alku_depo_v3"];
...
function renderOverview(){
  ...
  document.getElementById("overviewWarehouses").textContent=state.warehouses.length;
  document.getElementById("overviewKinds").textContent=all.length;
  document.getElementById("overviewPhysical").textContent=all.reduce((sum,x)=>sum+(Number(x.physical)||0),0);
  document.getElementById("overviewMissing").textContent=all.reduce((sum,x)=>sum+Math.max(0,(Number(x.system)||0)-(Number(x.physical)||0)),0);
  document.getElementById("overviewOpen").textContent=state.movements.filter(m=>m.status==="Açık").length;
}
...
document.getElementById("globalReportBtn").onclick=()=>{
  currentWarehouse=null;
  fillReportWarehouses();
  document.getElementById("reportWarehouse").value="ALL";
...
function buildReport(){
  ...
  const selectedWarehouse=wid==="ALL"?"Tüm Depolar":(warehouse(wid)?.name||wid);
  const createdAt=new Date().toLocaleString("tr-TR");
  const reportNo=`ALKU-${new Date().toISOString().slice(0,10).replaceAll("-","")}-${String(Date.now()).slice(-5)}`;
  document.getElementById("reportArea").innerHTML=`
    <section class="report-document">
      <div class="report-letterhead">
        <img src="logo.png" class="report-logo" alt="ALKÜ Logo">
        <div class="report-org">
          <h2>ALANYA ALAADDİN KEYKUBAT ÜNİVERSİTESİ</h2>
          <h3>Bilgi İşlem Daire Başkanlığı</h3>
        </div>
        <div class="report-meta">
          <p><strong>Rapor No:</strong> ${esc(reportNo)}</p>
          <p><strong>Tarih:</strong> ${esc(createdAt)}</p>
        </div>
      </div>
      <h1 class="report-main-title">${esc(reportTitle).toLocaleUpperCase("tr-TR")}</h1>
      <div class="report-context">
        <span><strong>Depo:</strong> ${esc(selectedWarehouse)}</span>
        <span><strong>Ürün çeşidi:</strong> ${items.length}</span>
        <span><strong>Sistem / Fiziksel:</strong> ${sys} / ${phy}</span>
        <span><strong>Eksik adet:</strong> ${miss}</span>
      </div>
      <table>
        <thead><tr>${reportHeaders.map(h=>`<th>${esc(h)}</th>`).join("")}</tr></thead>
        <tbody>${reportRows.length?reportRows.map(r=>`<tr>${r.map(v=>`<td>${esc(v)}</td>`).join("")}</tr>`).join(""):`<tr><td colspan="${reportHeaders.length}" class="empty">Kayıt bulunamadı.</td></tr>`}</tbody>
      </table>
      <div class="report-footer">Bu rapor ALKÜ Bilgi İşlem Daire Başkanlığı Depo Yönetim Sistemi tarafından oluşturulmuştur.</div>
    </section>`
}
...
function openFromHash(){
  ...
  if(parts.length>=2){
    const wid=parts[0],loc=parts[1];
    if(warehouse(wid)){
      openWarehouse(wid);
      if((warehouse(wid).locations||[]).includes(loc)){currentLocation=loc;renderInventory()}
      return true;
    }
  }
  if(warehouse(raw)){openWarehouse(raw);return true}
  return false;
}
window.addEventListener("hashchange",()=>{if(!openFromHash())showWarehouses()});
if(!openFromHash())showWarehouses();
